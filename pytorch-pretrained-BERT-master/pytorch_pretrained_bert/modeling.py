...
class BertModelMod(BertPreTrainedModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
    ...
    """
    def __init__(self, config):
        super(BertModel, self).__init__(config, attn_type, output_layer_type)
        self.embeddings = BertEmbeddings(config)
        self.encoder = BertEncoder(config)
        self.pooler = BertPooler(config)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True):
        ...
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return encoded_layers, pooled_output, context_hiddens, question_contexts
...
class BertForQuestionAnswering(BertPreTrainedModel):
    """ MODIFIED for project: BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    ...
    def __init__(self, config):
        ...
        #self.bert = BertModelMod(config,attn_type, output_layer_type)
